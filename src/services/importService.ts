...
export class ImportService {
  ...
  static async importOpenAPISpec(path: string): Promise<Collection> {
    try {
      const collection = await invoke<Collection>('import_openapi_spec', { path });
      return {
        ...collection,
        variables: [], // OpenAPI importer currently doesn't extract variables
      };
    } catch (error: any) {
      console.error('OpenAPI Import error:', error);
      throw new Error('Failed to import OpenAPI spec: ' + error);
    }
  }
